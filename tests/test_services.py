...
class ServiceTestCase(unittest.TestCase):
    ...
    def test_dashboard_snapshot_contains_expected_sections(self) -> None:
        regular = self.notes.create_note(title="Regular note")
        pinned = self.notes.create_note(title="Pinned note", is_pinned=1)
        archived = self.notes.create_note(title="Archived note", is_archived=1)
        vault = self.notes.create_note(title="Vault note", is_brain_vault=1)
        ...
        self.assertEqual(snapshot["random_brain_vault"].id, vault)
        self.assertEqual([note.id for note in snapshot["pinned_notes"]], [pinned])
        self.assertNotIn(archived, [note.id for note in snapshot["recent_notes"]])
        self.assertIn(regular, [note.id for note in snapshot["recent_notes"]])
